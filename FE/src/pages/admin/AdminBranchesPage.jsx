    ...
    >
      <div className="confirm-modal">
        ...
        <p className="confirm-modal__body">
          Chi nhánh <strong>{branch?.branchName}</strong> sẽ bị ngưng hoạt động. Nhân viên tại chi
          nhánh này sẽ không thể đăng nhập vào hệ thống. Không xóa cứng — bạn có thể kích hoạt lại
          sau.
        </p>
...
function BranchCard({ branch, onOpen, onEdit, onDeactivate, onReactivate }) {
  ...
  return (
    <div
      className={`branch-card ${branch.isActive ? 'branch-card--active' : 'branch-card--inactive'}`}
    >
      ...
      <div className="branch-card__actions">
        <button
          type="button"
          className="branch-card__btn branch-card__btn--stats"
        ...
        <PermissionGate permission="admin:branches:update">
          <button
            type="button"
            className="branch-card__btn branch-card__btn--edit"
    ...
    <div className="admin-branches__actions">
      {branches.length > 0 && (
        <span className="admin-branches__total-badge">
          ...
          {activeCount !== inactiveCount && inactiveCount > 0 ? ` · ${inactiveCount} ngừng` : ''}
        </span>
      )}
      <button
        className="btn btn--primary"
        onClick={() =>
          navigate('/admin/catalog/branches/new', { state: { fromListSearch: location.search } })
        }
      >
        <svg
          width="14"
          ...
          <div className="admin-branches__title-block">
            ...
            <div className="admin-branches__title-group">
              ...
              <p className="admin-branches__subtitle">
                Danh sách và thông tin các chi nhánh AutoGara
              </p>
            </div>
          </div>
          {headerActions}
          ...
          {branches.length === 0 ? (
            <div className="admin-branches__empty">
              ...
              <button
                className="btn btn--primary"
                onClick={() =>
                  navigate('/admin/catalog/branches/new', {
            ...
            <div className="admin-branches__cards">
              {branches.map((branch) => (
                <BranchCard
                  ...
                  branch={branch}
                  onOpen={(b) =>
                    navigate(`/admin/catalog/branches/${b.id}`, {
                      state: { fromListSearch: location.search },
                    })
                  }
                  onEdit={(b) =>
                    navigate(`/admin/catalog/branches/${b.id}/edit`, {
                      state: { fromListSearch: location.search },
                    })
                  }
