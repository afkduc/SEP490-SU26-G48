...
function getActionBadge(action) {
  ...
  if (upper.includes('ASSIGN') || upper.includes('GRANT')) {
    return { label: 'GÁN QUYỀN', bg: '#f3e8ff', color: '#3f3f46' };
  }
  ...
  if (upper.includes('PASSWORD') || upper.includes('RESET_PASS')) {
    return { label: 'ĐỔI MK', bg: '#f4f4f5', color: '#be185d' };
  }
  ...
  if (upper.includes('TOGGLE') || upper.includes('SWITCH') || upper.includes('STATUS')) {
    return { label: 'ĐỔI TRẠNG THÁI', bg: '#e4e4e7', color: '#09090b' };
  }
  // Import / Export
  if (upper.includes('IMPORT')) {
    return { label: 'NHẬP', bg: '#e4e4e7', color: '#3f3f46' };
  }
  if (upper.includes('EXPORT')) {
    return { label: 'XUẤT', bg: '#e4e4e7', color: '#3f3f46' };
  }
  // Upload / Download
  ...
  if (upper.includes('VIEW') || upper.includes('READ')) {
    return { label: 'XEM', bg: '#e4e4e7', color: '#09090b' };
  }
...
function getAlertStyle(alert) {
  ...
  if (
    ...
    return dark
      ? { bg: 'rgba(34,197,94,0.12)', border: '#166534', color: '#a1a1aa', iconBg: 'rgba(34,197,94,0.22)' }
      : { bg: '#f0fdf4', border: '#a1a1aa', color: '#3f3f46', iconBg: '#e4e4e7' };
  }
  if (
    raw === 'danger'
    || raw === 'error'
  ...
  if (
    ...
    return dark
      ...
      : { bg: '#fffbeb', border: '#d4d4d8', color: '#52525b', iconBg: '#f4f4f5' };
  }
  return dark
    ? { bg: '#162032', border: '#3f3f46', color: '#d4d4d8', iconBg: '#27272a' }
...
function LoginItem({ item }) {
  ...
  const loginStatusDot = (() => {
    if (item.actionType === 'LOGIN_FAILED' || item.status === 'failed') return '#27272a';
...
const QUICK_ACTIONS = [
  ...
  { to: '/admin/login-security', icon: <IconAlert />, label: 'Bảo mật đăng nhập', desc: 'Thiết bị + tín hiệu cảnh báo', accent: '#27272a' },
  { to: '/admin/logs', icon: <IconLog />, label: 'Nhật ký hoạt động', desc: 'Lịch sử thao tác', accent: '#52525b' },
  { to: '/admin/catalog', icon: <IconLogin />, label: 'Danh mục hệ thống', desc: 'Chi nhánh', accent: '#52525b' },
        ...
        <>
          ...
          <div className="dash-row-2">
            ...
            <div className="dash-widget">
              <SectionHeader
                dot="linear-gradient(135deg, #27272a, #52525b)"
