import { useState, useEffect, useCallback } from 'react';
import { getImportRequestsApi } from '../../services/importRequestApi';

/**
 * Quan ly trang thai trang Danh sach phieu nhap kho:
 * - Filter theo status / supplier / date range / search theo ma phieu.
 * - Phan trang page/limit.
 */
export function useImportRequests(initialBranchId) {
  const [params, setParams] = useState({
    branchId: initialBranchId,
    status: '',
    supplierId: '',
    fromDate: '',
    toDate: '',
    search: '',
    page: 1,
    limit: 20,
  });

  const [result, setResult] = useState({ items: [], total: 0, page: 1, limit: 20 });
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  const { branchId, status, supplierId, fromDate, toDate, search, page, limit } = params;

  useEffect(() => {
    setParams((current) => (
      current.branchId === initialBranchId
        ? current
        : { ...current, branchId: initialBranchId, page: 1 }
    ));
  }, [initialBranchId]);

  const fetchAll = useCallback(async () => {
    if (!branchId) {
      setLoading(false);
      return;
    }
    setLoading(true);
    setError(null);
    try {
      const res = await getImportRequestsApi({
        branchId, status, supplierId, fromDate, toDate, search, page, limit,
      });
      setResult(res || { items: [], total: 0, page, limit });
    } catch (err) {
      setError(err.message);
    } finally {
      setLoading(false);
    }
  }, [branchId, status, supplierId, fromDate, toDate, search, page, limit]);

  useEffect(() => {
    fetchAll();
  }, [fetchAll]);

  const setStatus = useCallback((v) => {
    setParams((p) => ({ ...p, status: v, page: 1 }));
  }, []);
  const setSupplierId = useCallback((v) => {
    setParams((p) => ({ ...p, supplierId: v, page: 1 }));
  }, []);
  const setFromDate = useCallback((v) => {
    setParams((p) => ({ ...p, fromDate: v, page: 1 }));
  }, []);
  const setToDate = useCallback((v) => {
    setParams((p) => ({ ...p, toDate: v, page: 1 }));
  }, []);
  const setSearch = useCallback((v) => {
    setParams((p) => ({ ...p, search: v, page: 1 }));
  }, []);
  const setPage = useCallback((v) => {
    setParams((p) => ({ ...p, page: v }));
  }, []);

  return {
    requests: result.items || [],
    total: result.total || 0,
    page: result.page || 1,
    limit: result.limit || 20,
    loading,
    error,
    params,
    setStatus,
    setSupplierId,
    setFromDate,
    setToDate,
    setSearch,
    setPage,
    refetch: fetchAll,
  };
}